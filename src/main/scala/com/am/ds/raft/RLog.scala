...
import com.am.ds.raft.rlog.{CommandApplier, LogAppender, SnapshotManager, MapDBPersistentLog}
import com.am.ds.raft.rpc.{ReadCommand, AppendEntries, LogEntry, WriteCommand}
...
class RLog(val cluster: Cluster, val stateMachine: StateMachine) extends Logging {
  ...
  def logEntry(index: Long, allowCompactedEntry: Boolean = false): Option[LogEntry] = {
    val entry: LogEntry = persistentLog.getEntry(index)
